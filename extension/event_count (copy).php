...
case FILL_CELLS: //fill cells (if edit, starts with Sessions)
	if (isset($_GET["getdesc"])) { //asking for the description of a cell
		...
		sav_state($_STATE); //'push' the working state back onto stack
	...
	case "SN":	//Info input starting with sessions
		...
		new_status($_STATE, STATE::CHANGE);
		break 1;
	default:
...
case STATE::CHANGE:
	$_STATE->EC_status = ""; //no line selected
	if (isset($_GET["reset"])) {
		$_STATE = get_state(NULL,$_STATE->thread); //go back to log_list
		break 1;
	}
	ob_clean();
	new_info($_DB, $_STATE);
