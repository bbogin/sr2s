...
function total_hours() { //for all selected projects (won't work in list mode)
	global $_DB, $_STATE;

	if ($_STATE->columns < 0) return; //list style

	$totals = array();
	for ($ndx=0; $ndx<$_STATE->columns; $ndx++) $totals[] = 0;
	...
	$stmt->bindValue(':fromdate', $_STATE->from_date->format('Y-m-d'), db_connect::PARAM_DATE);
	$stmt->bindValue(':todate', $_STATE->to_date->format('Y-m-d'), db_connect::PARAM_DATE);
	$stmt->execute();
	$one_day= new DateInterval('P1D'); //P=period, 1=number, D=days
	$next = clone $_STATE->from_date; //a DateTime object
	$ndx = 0;
...
//		break 1;
	...
	case STATE::CHANGE:
		...
		if (isset($_POST["actupd"])) {
			...
			new_hours($SCION);
			total_hours(); //re-calculate for all projects
		}
