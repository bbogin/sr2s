...
function logo_audit() {
	...
	$sql = "SELECT logo FROM ".$_STATE->prefix."a00_organization
			WHERE organization_id=".$_STATE->record_id.";";
	$stmt = $_STATE->query($sql);
	$stmt->bindColumn('logo', $logo, db_connect::PARAM_LOB);
	$stmt->fetch(PDO::FETCH_BOUND);
	$stmt->closeCursor();
	if (!is_null($logo)) {
		$_STATE->delete_BLOB($logo);
	}
	$oid = $_STATE->file_to_BLOB($_FILES["txtFile"]["tmp_name"]);
	$sql = "UPDATE ".$_STATE->prefix."a00_organization SET logo=:logo, logo_type=:logo_type
			WHERE organization_id=".$_STATE->record_id.";";
	$stmt = $_STATE->prepare($sql);
