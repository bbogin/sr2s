...
function active_rates() { //does user have an hourly rate set for each day in the period
	global $_DB, $_STATE;
...
function hours_send(&$state, &$HTML) {
	...
	for ($offset=$offset; $offset<=$maxcol; ++$offset) {
		...
		if ($state->rates[$offset] == 0) { //no hourly rate so no input
...
<!--
.tabs {
/*	border-bottom: .5em double #000000;*/
...
<?php
echo "<script type='text/javascript' src='".$redirect."/scripts/timelog.js'></script>\n";
}
...
if ($_STATE->columns < 0) {	//list style
	...
	for ($ndx=0,$day=clone $_STATE->from_date; $ndx<$_STATE->columns; $ndx++,$day->add($dayadd)) {
		$headrow .= "<th";
		if ($ndx < $_STATE->closedCols) $headrow .= " style='background-color:pink'";
		$headrow .= " width='50'>";
	...
	for ($ndx=0; $ndx<$_STATE->columns; $ndx++) {
		...
		if ($ndx < $_STATE->closedCols) $addrow .= " style='background-color:pink'";
		$addrow .= "></td>\n";
	}
} ?>
  <tr>
...
function echo_tabrow(&$_STATE, &$record, &$logs) {
	...
	echo "  <tr>\n";
	echo "    <td";
	if ($_STATE->closedCols < $_STATE->columns) echo " id='BN_".$row."' data-recid='".$row."' onclick=\"begin(this)\"";
	...
	echo "<tr>\n";
	echo "<td colspan='3'></td><td>Totals:</td>\n";
	$grand = 0;
	for ($ndx=0; $ndx<$_STATE->columns; $ndx++) {
		$grand += $totals[$ndx];
		echo "<td class='number'>".$totals[$ndx]."</td>";
	}
	echo "<td class='number'>Grand Total: ".$grand."</td>\n";
	echo "</tr>\n";
} // ---end TABULAR STYLE--- ?>
</table>
...
<form method="post" name="frmAction" id="frmAction_ID" action="<?php echo $_SERVER['SCRIPT_NAME']; ?>">
<br>You can
<button name="btnPut" type="submit" value="<?php echo $_STATE->person_id ?>" title="click here to download">Download</button>
