...
-- DROP TRIGGER `timesheets`.`a00_timestamp`;
...
delimiter //
CREATE TRIGGER `timesheets`.`a00_timestamp` BEFORE INSERT ON `timesheets`.`a00_organization`
FOR EACH ROW
...
-- DROP TRIGGER `timesheets`.`a10_timestamp`;
...
delimiter //
CREATE TRIGGER `timesheets`.`a10_timestamp` BEFORE INSERT ON `timesheets`.`a10_project`
FOR EACH ROW
BEGIN
  SET NEW.timestamp = UTC_TIMESTAMP();
...
-- DROP TRIGGER `timesheets`.`a12_timestamp`;
-- DROP TABLE `timesheets`.`a12_task`;
...
delimiter //
CREATE TRIGGER `timesheets`.`a12_timestamp` BEFORE INSERT ON `timesheets`.`a12_task`
FOR EACH ROW
  SET NEW.timestamp = UTC_TIMESTAMP()
//
delimiter ;
...
-- DROP TRIGGER `timesheets`.`a14_timestamp`;
-- DROP TABLE `timesheets`.`a14_subtask`;
...
delimiter //
CREATE TRIGGER `timesheets`.`a14_timestamp` BEFORE INSERT ON `timesheets`.`a14_subtask`
FOR EACH ROW
  SET NEW.timestamp = UTC_TIMESTAMP()
//
delimiter ;
...
-- DROP TRIGGER `timesheets`.`a20_timestamp`;
-- DROP TABLE `timesheets`.`a20_accounting`;
...
delimiter //
CREATE TRIGGER `timesheets`.`a20_timestamp` BEFORE INSERT ON `timesheets`.`a20_accounting`
FOR EACH ROW
  SET NEW.timestamp = UTC_TIMESTAMP()
//
delimiter ;
...
-- DROP TRIGGER `timesheets`.`a21_timestamp`;
-- DROP TABLE `timesheets`.`a21_account`;
...
delimiter //
CREATE TRIGGER `timesheets`.`a21_timestamp` BEFORE INSERT ON `timesheets`.`a21_account`
FOR EACH ROW
  SET NEW.timestamp = UTC_TIMESTAMP()
//
delimiter ;
...
-- DROP TRIGGER `timesheets`.`a30_timestamp`;
-- DROP TABLE `timesheets`.`a30_event`;
...
delimiter //
CREATE TRIGGER `timesheets`.`a30_timestamp` BEFORE INSERT ON `timesheets`.`a30_event`
FOR EACH ROW
  SET NEW.timestamp = UTC_TIMESTAMP()
//
delimiter ;
...
-- DROP TRIGGER `timesheets`.`b00_timestamp`;
-- DROP TABLE `timesheets`.`b00_timelog`;
...
delimiter //
CREATE TRIGGER `timesheets`.`b00_timestamp` BEFORE INSERT ON `timesheets`.`b00_timelog`
FOR EACH ROW
BEGIN
  SET NEW.timestamp = UTC_TIMESTAMP();
...
-- DROP TRIGGER `timesheets`.`b02_timestamp`;
-- DROP TABLE `timesheets`.`b02_activity`;
...
delimiter //
CREATE TRIGGER `timesheets`.`b02_timestamp` BEFORE INSERT ON `timesheets`.`b02_activity`
FOR EACH ROW
  SET NEW.timestamp = UTC_TIMESTAMP()
//
delimiter ;
...
-- DROP TRIGGER `timesheets`.`b10_timestamp`;
-- DROP TABLE `timesheets`.`b10_eventlog`;
...
delimiter //
CREATE TRIGGER `timesheets`.`b10_timestamp` BEFORE INSERT ON `timesheets`.`b10_eventlog`
FOR EACH ROW
  SET NEW.timestamp = UTC_TIMESTAMP()
//
delimiter ;
...
-- DROP TRIGGER `timesheets`.`c00_timestamp`;
-- DROP TABLE `timesheets`.`c00_person`;
...
delimiter //
CREATE TRIGGER `timesheets`.`c00_timestamp` BEFORE INSERT ON `timesheets`.`c00_person`
FOR EACH ROW
  SET NEW.timestamp = UTC_TIMESTAMP()
//
delimiter ;
...
-- DROP TRIGGER `timesheets`.`c02_timestamp`;
-- DROP TABLE `timesheets`.`c02_rate`;
...
delimiter //
CREATE TRIGGER `timesheets`.`c02_timestamp` BEFORE INSERT ON `timesheets`.`c02_rate`
FOR EACH ROW
BEGIN
  SET NEW.timestamp = UTC_TIMESTAMP();
...
-- DROP TRIGGER `timesheets`.`c10_timestamp`;
-- DROP TABLE `timesheets`.`c10_person_organization`;
...
delimiter //
CREATE TRIGGER `timesheets`.`c10_timestamp` BEFORE INSERT ON `timesheets`.`c10_person_organization`
FOR EACH ROW
  SET NEW.timestamp = UTC_TIMESTAMP()
//
delimiter ;
...
-- DROP TRIGGER `timesheets`.`c20_timestamp`;
-- DROP TABLE `timesheets`.`c20_person_permit`;
...
delimiter //
CREATE TRIGGER `timesheets`.`c20_timestamp` BEFORE INSERT ON `timesheets`.`c20_person_permit`
FOR EACH ROW
  SET NEW.timestamp = UTC_TIMESTAMP()
//
delimiter ;
...
-- DROP TRIGGER `timesheets`.`d01_timestamp`;
-- DROP TABLE `timesheets`.`d01_permit`;
...
delimiter //
CREATE TRIGGER `timesheets`.`d01_timestamp` BEFORE INSERT ON `timesheets`.`d01_permit`
FOR EACH ROW
  SET NEW.timestamp = UTC_TIMESTAMP()
//
delimiter ;
...
-- DROP TABLE `timesheets`.`d02_currency`;
...
-- DROP TRIGGER `timesheets`.`d10_timestamp`;
-- DROP TABLE `timesheets`.`d10_preferences`;

CREATE TABLE `timesheets`.`d10_preferences` (
...
delimiter //
CREATE TRIGGER `timesheets`.`d10_timestamp` BEFORE INSERT ON `timesheets`.`d10_preferences`
FOR EACH ROW
  SET NEW.timestamp = UTC_TIMESTAMP()
//
delimiter ;
...
-- DROP VIEW `timesheets`.`v00_timelog`;

CREATE OR REPLACE VIEW `timesheets`.`v00_timelog` AS 
...
-- DROP VIEW `timesheets`.`v10_logreport`;

CREATE OR REPLACE VIEW `timesheets`.`v10_logreport` AS 
...
-- DROP VIEW `timesheets`.`v12_taskreport`;

CREATE OR REPLACE VIEW `timesheets`.`v12_taskreport` AS 
