...
-- DROP TRIGGER `timesheets`.`d_a00_timestamp`;
-- DROP TABLE `d_a00_organization`;

CREATE TABLE `timesheets`.`d_a00_organization` (
...
delimiter //
CREATE TRIGGER `timesheets`.`d_a00_timestamp` BEFORE INSERT ON `timesheets`.`d_a00_organization`
FOR EACH ROW
...
-- DROP TRIGGER `timesheets`.`d_a10_timestamp`;
-- DROP TABLE `d_a10_project`;

CREATE TABLE `timesheets`.`d_a10_project` (
...
delimiter //
CREATE TRIGGER `timesheets`.`d_a10_timestamp` BEFORE INSERT ON `timesheets`.`d_a10_project`
FOR EACH ROW
BEGIN
  SET NEW.timestamp = UTC_TIMESTAMP();
...
-- DROP TRIGGER `timesheets`.`d_a12_timestamp`;
-- DROP TABLE `timesheets`.`d_a12_task`;

CREATE TABLE `timesheets`.`d_a12_task` (
...
delimiter //
CREATE TRIGGER `timesheets`.`d_a12_timestamp` BEFORE INSERT ON `timesheets`.`d_a12_task`
FOR EACH ROW
  SET NEW.timestamp = UTC_TIMESTAMP()
//
delimiter ;
...
-- DROP TRIGGER `timesheets`.`d_a14_timestamp`;
-- DROP TABLE `timesheets`.`d_a14_subtask`;

CREATE TABLE `timesheets`.`d_a14_subtask` (
...
delimiter //
CREATE TRIGGER `timesheets`.`d_a14_timestamp` BEFORE INSERT ON `timesheets`.`d_a14_subtask`
FOR EACH ROW
  SET NEW.timestamp = UTC_TIMESTAMP()
//
delimiter ;
...
-- DROP TRIGGER `timesheets`.`d_a20_timestamp`;
-- DROP TABLE `timesheets`.`d_a20_accounting`;

CREATE TABLE `timesheets`.`d_a20_accounting` (
...
delimiter //
CREATE TRIGGER `timesheets`.`d_a20_timestamp` BEFORE INSERT ON `timesheets`.`d_a20_accounting`
FOR EACH ROW
  SET NEW.timestamp = UTC_TIMESTAMP()
//
delimiter ;
...
-- DROP TRIGGER `timesheets`.`d_a21_timestamp`;
-- DROP TABLE `timesheets`.`d_a21_account`;

CREATE TABLE `timesheets`.`d_a21_account` (
...
delimiter //
CREATE TRIGGER `timesheets`.`d_a21_timestamp` BEFORE INSERT ON `timesheets`.`d_a21_account`
FOR EACH ROW
  SET NEW.timestamp = UTC_TIMESTAMP()
//
delimiter ;
...
-- DROP TRIGGER `timesheets`.`d_a30_timestamp`;
-- DROP TABLE `timesheets`.`d_a30_event`;

CREATE TABLE `timesheets`.`d_a30_event` (
...
delimiter //
CREATE TRIGGER `timesheets`.`d_a30_timestamp` BEFORE INSERT ON `timesheets`.`d_a30_event`
FOR EACH ROW
  SET NEW.timestamp = UTC_TIMESTAMP()
//
delimiter ;
...
-- DROP TRIGGER `timesheets`.`d_b00_timestamp`;
-- DROP TABLE `timesheets`.`d_b00_timelog`;

CREATE TABLE `timesheets`.`d_b00_timelog` (
...
delimiter //
CREATE TRIGGER `timesheets`.`d_b00_timestamp` BEFORE INSERT ON `timesheets`.`d_b00_timelog`
FOR EACH ROW
BEGIN
  SET NEW.timestamp = UTC_TIMESTAMP();
...
-- DROP TRIGGER `timesheets`.`d_b02_timestamp`;
-- DROP TABLE `timesheets`.`d_b02_activity`;

CREATE TABLE `timesheets`.`d_b02_activity` (
...
delimiter //
CREATE TRIGGER `timesheets`.`d_b02_timestamp` BEFORE INSERT ON `timesheets`.`d_b02_activity`
FOR EACH ROW
  SET NEW.timestamp = UTC_TIMESTAMP()
//
delimiter ;
...
-- DROP TRIGGER `timesheets`.`d_b10_timestamp`;
-- DROP TABLE `timesheets`.`d_b10_eventlog`;

CREATE TABLE `timesheets`.`d_b10_eventlog` (
...
delimiter //
CREATE TRIGGER `timesheets`.`d_b10_timestamp` BEFORE INSERT ON `timesheets`.`d_b10_eventlog`
FOR EACH ROW
  SET NEW.timestamp = UTC_TIMESTAMP()
//
delimiter ;
...
-- DROP TRIGGER `timesheets`.`d_c00_timestamp`;
-- DROP TABLE `timesheets`.`d_c00_person`;

CREATE TABLE `timesheets`.`d_c00_person` (
...
delimiter //
CREATE TRIGGER `timesheets`.`d_c00_timestamp` BEFORE INSERT ON `timesheets`.`d_c00_person`
FOR EACH ROW
  SET NEW.timestamp = UTC_TIMESTAMP()
//
delimiter ;
...
-- DROP TRIGGER `timesheets`.`d_c02_timestamp`;
-- DROP TABLE `timesheets`.`d_c02_rate`;

CREATE TABLE `timesheets`.`d_c02_rate` (
...
delimiter //
CREATE TRIGGER `timesheets`.`d_c02_timestamp` BEFORE INSERT ON `timesheets`.`d_c02_rate`
FOR EACH ROW
BEGIN
  SET NEW.timestamp = UTC_TIMESTAMP();
...
-- DROP TRIGGER `timesheets`.`d_c10_timestamp`;
-- DROP TABLE `timesheets`.`d_c10_person_organization`;

CREATE TABLE `timesheets`.`d_c10_person_organization` (
...
delimiter //
CREATE TRIGGER `timesheets`.`d_c10_timestamp` BEFORE INSERT ON `timesheets`.`d_c10_person_organization`
FOR EACH ROW
  SET NEW.timestamp = UTC_TIMESTAMP()
//
delimiter ;
...
-- DROP TRIGGER `timesheets`.`d_c20_timestamp`;
-- DROP TABLE `timesheets`.`d_c20_person_permit`;

CREATE TABLE `timesheets`.`d_c20_person_permit` (
...
delimiter //
CREATE TRIGGER `timesheets`.`d_c20_timestamp` BEFORE INSERT ON `timesheets`.`d_c20_person_permit`
FOR EACH ROW
  SET NEW.timestamp = UTC_TIMESTAMP()
//
delimiter ;
...
-- DROP TRIGGER `timesheets`.`d_d01_timestamp`;
-- DROP TABLE `timesheets`.`d_d01_permit`;

CREATE TABLE `timesheets`.`d_d01_permit` (
...
delimiter //
CREATE TRIGGER `timesheets`.`d_d01_timestamp` BEFORE INSERT ON `timesheets`.`d_d01_permit`
FOR EACH ROW
  SET NEW.timestamp = UTC_TIMESTAMP()
//
delimiter ;
...
-- DROP TABLE `timesheets`.`d_d02_currency`;

CREATE TABLE `timesheets`.`d_d02_currency` (
...
-- DROP TRIGGER `timesheets`.`d_d10_timestamp`;
-- DROP TABLE `timesheets`.`d_d10_preferences`;

CREATE TABLE `timesheets`.`d_d10_preferences` (
...
delimiter //
CREATE TRIGGER `timesheets`.`d_d10_timestamp` BEFORE INSERT ON `timesheets`.`d_d10_preferences`
FOR EACH ROW
  SET NEW.timestamp = UTC_TIMESTAMP()
//
delimiter ;
...
-- DROP VIEW `timesheets`.`d_v00_timelog`;

CREATE OR REPLACE VIEW `timesheets`.`d_v00_timelog` AS 
...
-- DROP VIEW `timesheets`.`d_v10_logreport`;

CREATE OR REPLACE VIEW `timesheets`.`d_v10_logreport` AS 
...
-- DROP VIEW `timesheets`.`d_v12_taskreport`;

CREATE OR REPLACE VIEW `timesheets`.`d_v12_taskreport` AS 
