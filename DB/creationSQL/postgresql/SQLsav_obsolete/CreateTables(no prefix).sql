...
COMMENT ON COLUMN b10_eventelog."timestamp" IS 'Time is UTC, alias GMT, alias Greenwich Mean Time';
...
the connecting point for person properties that are organization specific, eg. permits';
...
COMMENT ON TABLE c20_person_permit IS 'connect a person to a permission (organization specific) - 
many-to-many - through the person/organization table';
...
CREATE OR REPLACE VIEW v00_timelog AS 
 SELECT b00.timelog_id, b00.logdate, b00.hours, b02.activity_id, b02.description AS activity, a14.subtask_id,
        a14.name AS subtask, a14.description AS subtask_desc, a14.extension, a12.task_id, a12.name AS task,
        a12.description AS task_desc, a10.project_id, a10.name AS project, a10.description AS project_desc,
        a21.account_id, a21.name AS account, a21.description AS account_desc, a00.organization_id, c00.person_id
...
CREATE OR REPLACE VIEW v12_taskreport AS 
 SELECT project_idref AS project_id, task_id, a12.name AS taskname, a12.description AS task_desc, budget,
        ...
        a12.inactive_asof AS subtask_inactive_asof
